                                    ...
                                    <div class="border border-dark text-center bg-light fs-14">
                                        ...
                                        <div class="select-product-list">
                                            ...
                                            <div class="my-2">
                                                ...
                                                <h6 class="fs-14">{{\General::currency_format($sl['Price'],0,'',',')}}</h6>
                                            </div>
                                            @endforeach
                                        </div>
...
<script>
        ...
        if (selectedSolitaire) {
            $.ajax({
                ...
                success: function(res) {
                    ...
                    if (res.flag === 1) {
                        window.location.href = base_url + '/design-your-own-diamond-earrings?cat=' + category + '&selected=2';
                    } else {
    ...
    function filterDiamondData() {
        ...
        if (filterParam.cut && filterParam.cut.length > 0) {
            filteredData = filteredData.filter(obj => filterParam.cut.includes(obj[0].Cut));
        }
        if (filterParam.color && filterParam.color.length > 0) {
            filteredData = filteredData.filter(obj => filterParam.color.includes(obj[0].Color));
        }
        if (filterParam.clarity && filterParam.clarity.length > 0) {
            filteredData = filteredData.filter(obj => filterParam.clarity.includes(obj[0].Clarity));
        }
        if (filterParam.polish && filterParam.polish.length > 0) {
            filteredData = filteredData.filter(obj => filterParam.polish.includes(obj[0].Pol));
        }
        if (filterParam.fl && filterParam.fl.length > 0) {
            filteredData = filteredData.filter(obj => filterParam.fl.includes(obj[0].DisplayFl));
        }
        if (filterParam.lab && filterParam.lab.length > 0) {
            filteredData = filteredData.filter(obj => filterParam.lab.includes(obj[0].Cert));
        }
        if (filterParam.sym && filterParam.sym.length > 0) {
            filteredData = filteredData.filter(obj => filterParam.sym.includes(obj[0].Sym));
        }
        if (filterParam.shape && filterParam.shape.length > 0) {
            filteredData = filteredData.filter(obj => filterParam.shape.includes(obj[0].DisplayShape));
        }
        if (filterParam.carat) {
            filteredData = filteredData.filter(obj => filterParam.carat.min <= obj[0].Weight && filterParam.carat.max >= obj[0].Weight);
        }
        ...
        if (filterParam.table) {
            filteredData = filteredData.filter(obj => filterParam.table.min <= obj[0].Table && filterParam.table.max >= obj[0].Table);
        }
        if (filterParam.depth) {
            filteredData = filteredData.filter(obj => filterParam.depth.min <= obj[0].TopDepth && filterParam.depth.max >= obj[0].TopDepth);
        }
        currentPage = 1
