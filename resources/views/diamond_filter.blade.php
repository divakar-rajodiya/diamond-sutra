...
if($diamondSelected && $selectedDiamond != null){
$displayFirstStep = 0;
$displaySecondStep = 1;
                                    ...
                                    <div class="border border-dark text-center bg-light fs-14">
                                        ...
                                        <div class="select-product-list">
                                            <div class="my-2">
                                                ...
                                                <h6 class="fs-14">{{\General::currency_format($selectedDiamond['product_buy_price'],0,'',',')}}</h6>
                                            </div>
                                        </div>
                                        <div class="border border-top p-4">
                                            <h6><b>TOTAL</b><br>
                                                {{\General::currency_format($selectedDiamond['product_buy_price'],0,'',',')}}
                                            </h6>
                    ...
                    <div class="col-lg-5">
                        <div>
                            ...
                            <div class="slider-thumb">
                                @if($selectedProduct['images']['image1'])
                                ...
                                @endif
                                @if($selectedProduct['images']['image3'])
                                ...
                                @endif
                                @if($selectedProduct['images']['model_image'])
...
<script>
    ...
    var makingChargeDiscountList = $('#making-charge-discount-list').val();
    if(makingChargeDiscountList != undefined) {
        makingChargeDiscountList = JSON.parse(makingChargeDiscountList);
    }
    ...
    $(document).on('click', '#btn-buy-now', async function() {
        ...
        let solitaireDetail = await JSON.parse(localStorage.getItem('selected_solitaire')) || {};
        let mount_id = solitaireDetail.cart_id;
        settingDetail.mount_id = mount_id;
        solitaireDetail.mount_id = mount_id;
        
        console.log('selected solitaire : ', solitaireDetail);
        console.log('selected setting: ', settingDetail);

        let cart = JSON.parse(localStorage.getItem('cart')) || {};

        for (let key in cart) {
            if (cart[key].mount_id == mount_id && cart[key].cart_id != mount_id) {
                delete cart[key];
            }
        }   
        ...
        if (selectedSolitaire) {
            $.ajax({
                ...
                success: function(res) {
                    ...
                    if (res.flag === 1) {
                        window.location.href = base_url + '/design-your-own-solitaire-' + category + '?cat=' + category + '&selected=2';
                    } else {
                        Toast(res.msg, 3000, res.flag);
                    }
                },
