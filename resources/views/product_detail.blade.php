  ...
  <div class="container">
    <div class="row gy-4">
      <div class="col-lg-5">
        ...
        <div>
          ...
          <input type="hidden" id="making-charge-discount-list" value="{{json_encode($product['making_charge_discount_list'])}}">
          <input type="hidden" id="diamond-price-list" value="{{json_encode($product['diamond_price_list'])}}">
          <input type="hidden" id="diamond-discount-per" value="{{$product['diamond_discount_per']}}">
          ...
          <input type="hidden" id="diamond_price_IJ_SI" value="{{$product['price_IJ_SI']}}">
          <input type="hidden" id="diamond_price_GH_SI" value="{{$product['price_GH_SI']}}">
          <input type="hidden" id="diamond_price_GH_VS" value="{{$product['price_GH_VS']}}">
          <input type="hidden" id="diamond_EF_VVS" value="{{$product['EF_VVS']}}">
          ...
          <input type="hidden" id="selected-product-name" value="{{$product['name']}}">
          ...
          <div class="slider-content">
            <div class="xzoom-container h-100">
              <a data-fancybox-trigger="gallery" href="javascript:;">
                <img class="xzoom h-100 w-100 image-1" id="xzoom-default" src="{{url('public/assets/img/product/').'/'.$product['product_sku'].'/'.$product['product_sku'].'_W1.webp'}}" xoriginal="{{url('public/assets/img/product/').'/'.$product['product_sku'].'/'.$product['product_sku'].'_W1.webp'}}" />
              </a>
            </div>
          </div>
          @if($is_video == true)
          ...
          <div class="slider-thumb">
            ...
            <div class="slide"><img src="{{url('public/assets/img/product/').'/'.$product['product_sku'].'/'.$product['product_sku'].'_W1.webp'}}" alt="" class="img-fluid thumb-image thumb-image-1"></div>
            @endif
            ...
            <div class="slide"><img src="{{url('public/assets/img/product/').'/'.$product['product_sku'].'/'.$product['product_sku'].'_W2.webp'}}" alt="" class="img-fluid thumb-image thumb-image-2"></div>
            @endif
            @if($product['images']['image3'])
            <div class="slide"><img src="{{url('public/assets/img/product/').'/'.$product['product_sku'].'/'.$product['product_sku'].'_W3.webp'}}" alt="" class="img-fluid thumb-image thumb-image-3"></div>
            @endif
            @if($product['images']['image4'])
            <div class="slide"><img src="{{url('public/assets/img/product/').'/'.$product['product_sku'].'/'.$product['product_sku'].'_W4.webp'}}" alt="" class="img-fluid thumb-image thumb-image-4"></div>
            @endif
            @if($product['images']['model_image'])
            <div class="slide"><img src="{{url('public/assets/img/product/').'/'.$product['product_sku'].'/'.$product['product_sku'].'_Model_W.webp'}}" alt="" class="img-fluid thumb-image thumb-model-image"></div>
            @endif
          </div>
      ...
      <div class="offset-lg-1 col-lg-6">
        <div class="product-info-panel">
          ...
          <div class="cart-btn-group my-4">
            ...
            <button id="btn-add-to-cart" class="btn primary-btn large-btn w-50 mb-3">Buy Now</button>
          </div>
          ...
          <div class="shipping-detail">
            ...
            <img src="{{url('public/assets/img/diamond-sutra-promises.png')}}" class="img-fluid" alt="">
            ...
            <a href="{{url('').'/'.$item['category']['name'].'/'.$item['product_sku'].'/'.$item['default_color'].'-gold'.'/'.str_replace(' ', '-',$item['name'])}}">
              <div class="single-product-card">
                <div class="product-img h-auto">
                  ...
                  <img src="{{url('public/assets/img/product/').'/'.$item['product_sku'].'/'.$item['product_sku'].'_'.$color.'1.webp'}}" class="image-main h-auto" alt="">
                  <!-- <img src="{{url('public/assets/img/product/').'/'.$item['product_sku'].'/'.$item['product_sku'].'_Model_'.$color.'.webp'}}" class="image-hover" alt=""> -->
                </div>
    ...
    <div class="item-review mb-4 mb-md-5 mt-5">
      ...
      <div class="product-review-wrapper">
        <div class="row gy-3">
          <div class="col-12">
            @if($product['total_reviews'] < 1) <h4> No review found </h4>
              @else
              ...
              <div class="d-flex gap-3">
                ...
                <div class="review-content">
                  ...
                  <div class="review-img-list">
                    <ul class="list-unstyled d-flex flex-wrap gap-2">
                      <li>
                        <img src="{{url('public/assets/img/rign-img-1.webp')}}" alt="">
                      </li>
                    </ul>
                  </div>
                </div>
...
<script>
  ...
  var makingChargeDiscountList = JSON.parse($('#making-charge-discount-list').val());
  var diamondPriceList = JSON.parse($('#diamond-price-list').val());
  var defaultDiamondPriceList = {
    IJ_SI: 0,
    GH_SI: 0,
    GH_VS: 0,
    EF_VVS: 0
  ...
  $(document).ready(function() {
    defaultDiamondPriceList = {
      IJ_SI: $('#diamond_price_IJ_SI').val(),
      GH_SI: $('#diamond_price_GH_SI').val(),
      GH_VS: $('#diamond_price_GH_VS').val(),
      EF_VVS: $('#diamond_price_EF_VVS').val()
    };
  });
  ...
  function updateData() {
    ...
    $('.thumb-image-4').attr('src', defaultImagePath + '_' + selectedColorCode + '4.webp');
  ...
  // async function getProductPrice(prodParam) {
  //   return new Promise((resolve, reject) => {
  //     prodParam._token = token;
  //     $.ajax({
  //       type: 'POST',
  //       url: base_url + '/get-product-custom-price', // Replace with your endpoint
  //       data: prodParam,
  //       success: function(res) {
  //         // Redirect the user to the checkout page
  //         console.log(res);
  ...
  $(document).on('click', '#btn-add-to-cart', async () => {
    ...
    const productSku = $('#product-sku').val();
    const selectedProductColor = $('#selected-product-color').val();
    const selectedProductName = $('#selected-product-name').val();
    const selectedProductSize = $('#selected-product-size').val();
    const selectedDiamondQuality = $('#selected-diamond-quality').val();
    const selectedGoldCarat = $('#selected-gold-carat').val();
    const selectedSolitaire = $('#selected-solitaire').val();
    ...
    let custProdParam = {
      sku: productSku,
      product_type: 'product', //  solitaire or product
      color: selectedColor,
      size: selectedProductSize,
    ...
    // ajax call here to get product price
    let res = await getProductPrice(custProdParam);
    let productDetails = await res.data;

    // Prepare the data object
    var productData = productDetails;
    productData.item_type = 'primary';



    // console.log('cart id : ',productData.cart_id);
    let cart = JSON.parse(localStorage.getItem('cart')) || {};
    let sort_index = Object.keys(cart).length;
    productData.sort_index = sort_index;

    if (cart[productData.cart_id]) {
      cart[productData.cart_id].quantity++;
    } else {
      // console.log('before cart:', cart);
      cart[productData.cart_id] = productData;
  ...
  function removeAddOnProduct(mainProductId, addOnProductId) {
    let cart = JSON.parse(localStorage.getItem('cart')) || {};

    if (cart[mainProductId]) {
      cart[mainProductId].addOns = cart[mainProductId].addOns.filter(addOn => addOn.id !== addOnProductId);
      localStorage.setItem('cart', JSON.stringify(cart));
      alert('Add-on product removed from cart');
    } else {
      alert('Main product not found in cart');
    }
  }
  ...
  function updateProductPrice() {
    ...
    if ($('#product-size-chart').val() == 'null') {
      ...
      makingChargeActual = makingChargeDiscountList[selectedCarat][selectedSize]['actual_price'];
    }
    ...
    if (is_diamond === 'yes') {
      console.warn(`price ${selectedDiamond} : `, defaultDiamondPriceList[selectedDiamond]);
      if (defaultDiamondPriceList[selectedDiamond] != undefined && defaultDiamondPriceList[selectedDiamond] != 0) {
        let diamondDiscountPer = parseInt($('#diamond-discount-per').val());
        diamondBuyPrice = defaultDiamondPriceList[selectedDiamond];
        diamondDiscount = diamondDiscountPer;

        // Calculate diamondBasePrice
        if (diamondDiscountPer > 0) {
          diamondBasePrice = Math.round(diamondBuyPrice / (1 - diamondDiscountPer / 100));
        } else {
          diamondBasePrice = diamondBuyPrice; // No discount
        }
      } else {
    ...
    totalAmount = (Math.round(parseFloat(goldPrice)) + Math.round(parseFloat(stonePrice)) + Math.round(parseFloat(diamondBasePrice)) + Math.round(parseFloat(makingChargeActual)) + Math.round(parseFloat(solitairePresetPrice)));
