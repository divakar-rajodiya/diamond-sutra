    ...
    <div class="container">
        <div class="row gy-3 gy-md-4">
            ...
            <div class="col-lg-6">
                ...
                <div>
                    <div class="slider-content" id="slider-section-1">
                        @php $defaultImage = 'http://dev.diamondsutra.in/public/assets/img/solitaire/'.$solitaire[0]['DisplayShape'].'.jpg'; @endphp
                        <div class="xzoom-container h-100">
                            <a data-fancybox-trigger="gallery" href="javascript:;">
                                <img class="xzoom h-100 w-100 image-1" id="xzoom-default" src="{{$solitaire[0]['ImageLink']}}" onerror="this.onerror=null; this.src='<?php echo $defaultImage; ?>';"/>
                            </a>
                        </div>
                    </div>
                    ...
                    <div class="slider-thumb">
                        ...
                        <div class="slide"><img src="{{$solitaire[0]['ImageLink']}}" alt="" class="img-fluid thumb-image thumb-image-1" data-id="1" onerror="this.onerror=null; this.src='<?php echo $defaultImage; ?>';"></div>
                ...
                <div class="product-info-panel">
                    ...
                    <div class="product-info-price mb-3">
                        <span class="subtotal-price">{{\General::currency_format($solitaire[0]['product_buy_price'])}}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="col-lg-6">
                ...
                <div>
                    @php $defaultImage1 = 'http://dev.diamondsutra.in/public/assets/img/solitaire/'.$solitaire[1]['DisplayShape'].'.jpg'; @endphp
                    <div class="slider-content" id="slider-section-2">
                        <div class="xzoom-container h-100">
                            <a data-fancybox-trigger="gallery" href="javascript:;">
                                <img class="xzoom h-100 w-100 image-1" id="xzoom-default" src="{{$solitaire[1]['ImageLink']}}" xoriginal="{{$solitaire[1]['ImageLink']}}" onerror="this.onerror=null; this.src='<?php echo $defaultImage1; ?>';"/>
                            </a>
                        </div>
                    </div>
                    ...
                    <div class="slider-thumb">
                        <div class="slide"><img src="{{$solitaire[1]['ImageLink']}}" alt="" class="img-fluid thumb-image thumb-image-1" data-id="2" onerror="this.onerror=null; this.src='<?php echo $defaultImage1; ?>';"></div>
                ...
                <div class="product-info-panel">
                    ...
                    <div class="product-info-price mb-3">
                        <span class="subtotal-price">{{\General::currency_format($solitaire[1]['product_buy_price'])}}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="col-12">
                <div class="cart-btn-group my-4 text-center">
                    <button class="btn primary-btn large-btn " id="select-diamond">Add to cart</button>
...
<script>
    ...
    $(document).on('click', '#select-diamond', async function() {
                console.log(diamond);
                // localStorage.setItem('selected_solitaire', JSON.stringify(diamond));

                let custProdParam = [{
                        ref_no: diamond[0].RefNo,
                        cert_no: diamond[0].CertNo,
                        product_type: 'loose_solitaire_pair' //  solitaire, product, loose_solitaire, loose_pair
                    },
                    {
                        ref_no: diamond[1].RefNo,
                        cert_no: diamond[1].CertNo,
                        product_type: 'loose_solitaire_pair' //  solitaire, product, loose_solitaire, loose_pair
                    },
                ];
                try {
                    // console.log('cust param before : ', custProdParam);
                    let res = await getProductPairPrice(custProdParam);
                    if (res.flag === 1) {
                        let productDetails = await res.data;
                        let cart = JSON.parse(localStorage.getItem('cart')) || {};

                        productDetails.forEach(solitaireDetail => {
                        ...
                        if (!cart[productDetails[0].RefNo]) {
                            productDetails[0].sort_index = sort_index;
                            cart[productDetails[0].RefNo] = productDetails[0];
                            sort_index++;
                        }
                        if (!cart[productDetails[1].RefNo]) {
                            productDetails[1].sort_index = sort_index;
                            cart[productDetails[1].RefNo] = productDetails[1];
                        }
                        localStorage.setItem('cart', JSON.stringify(cart));
                        refreshCartLogo();
                    ...
                    async function getProductPairPrice(prodParam) {
                        return new Promise((resolve, reject) => {
                            let token = $('#token').val();
                            prodParam._token = token;
                            $.ajax({
                                type: 'POST',
                                url: base_url + '/get-pair-custom-price', // Replace with your endpoint
                                data: {
                                    cartData: prodParam,
                                    _token: token
                                },
